...
export class PasswordRecoveryService {
  ...
  public async resetPassword(dto: ResetPasswordDto) {
    ...
    if (!existingUser) {
      throw new NotFoundException(
        'Пользователь не найден.Проверте введенный адрес электронной почты.',
      );
    }
  ...
  public async newPassword(dto: NewPasswordDto, token: string) {
    ...
    if (!existingToken) {
      throw new NotFoundException('Токен не найден.Проверте правильность введенного токена.');
    }

    const hasExpired = new Date(existingToken.expiresIn) < new Date();

    if (hasExpired) {
      throw new BadRequestException('Токен истек. Запросите новый токен для сброса пароля');
    }
